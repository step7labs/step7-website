  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Work — Step7Labs" },
      {
        ...
        content:
          "Selected projects with measurable outcomes — fintech, SaaS, e-commerce, brand.",
      },
    ],
  }),
  component: WorkPage,
  ...
  return (
    <>
      <PageHero
        ...
        title={<>Things we've <em className="italic text-foreground/70">shipped.</em></>}
      ...
      <section className="py-16 md:py-24">
        <div className="mx-auto max-w-7xl px-6 md:px-10 grid md:grid-cols-2 gap-10 md:gap-12">
          {list.map((p) => (
            ...
            >
              <div className={`relative aspect-[4/3] bg-gradient-to-br ${p.color} overflow-hidden rounded-sm`}>
                <div className="absolute inset-0 grid place-items-center">
                  <div className="font-display text-6xl md:text-8xl text-white/10 tracking-tight">
                    {p.name.split(" ")[0]}
                  </div>
                </div>
                <div className="absolute top-5 left-5 font-mono-tech text-white/60">{p.category}</div>
              ...
              <div className="mt-6 flex items-start justify-between gap-6">
                ...
                <div className="font-mono-tech text-muted-foreground whitespace-nowrap">{p.metric}</div>
              </div>
            </Link>
          ))}
        </div>
